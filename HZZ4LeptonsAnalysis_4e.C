...
void HZZ4LeptonsAnalysis::Loop(const Char_t *output)
{
   ...
   std::cout << datasetBase.Data() << std::endl;
   //pT gen reweighting
   char histotitle[500];
...
*/    
   ...
   */
   ...
   std::cout << "Branches booked.\n";
   float newweight=1.;
   
   // New tree with clone of events passing the final selection
   TFile *skimfile = new TFile(eventsOUT,"recreate");
    std::cout << "Skim file created.\n";
   // Clone tree for final events
   //TTree *finaltree = fChain->CloneTree(0); //This is broken if *fChain is a TChain instead of a TTree
   //Instead, load the first entry, get the tree, and clone that:
    if(fChain->LoadTree(0) < 0) {
        //Failed to load tree. This is fatal.
        std::cout << "Failed to load tree. Exiting." << std::endl;
        exit(7);
    }
    TTree *finaltree = fChain->GetTree()->CloneTree(0);
   ...
   size_t bad_events=0;
   for (Long64_t jentry=0;jentry<fChain->GetEntriesFast();++jentry) {//jentry<nentries;jentry++) {
      Long64_t ientry = LoadTree(jentry);
      if (ientry < 0) break;
      nb = fChain->GetEntry(jentry); 
      if(nb < 0) { // Read error
        std::cerr << "Read error on event " << jentry << ". Will try again.\n";
        for(unsigned int i=60;i<=180;i+=60) {
            //If it was a temporary network error, maybe waiting for a bit will help
            waited+=i;
            unsigned int j=i;
            while(j=sleep(j));
            nb=fChain->GetEntry(jentry);
        ...
        if (nb < 0) {
            std::cerr << "Could not read event " << jentry << ". Exiting.\n";
            exit(1);
        }
      }
      nbytes += nb;
      if(MC_type=="NO" && !good_lumi(Run,LumiSection)) {
        ++bad_events;
        continue;
      } else {
        ++good_events;
...
//    std::cout << "Partons" << iparton << " " << iparton << std::endl;
	  ...
	  //(safeAccess(LHE_PARTON_PT))[iparton] = -2;
	}
	else{
...
//      float pFill[11];for(int pf=0;pf<11;pf++)pFill[11]=-999.;
      ...
      //
      if (isSignal || isggH){
	if( (safeAccess(MC_PDGID))[3] == 11 && (safeAccess(MC_PDGID))[4] == -11  
	    && (safeAccess(MC_PDGID))[5] == 11 && (safeAccess(MC_PDGID))[6] == -11 ){
	  is4e=true;
	  ++N_01 ;  // fill counter 
     ...
     */
 	...
 	if( (safeAccess(RECOELE_PT))[i] > 7. 
	    ...
	    //&& (safeAccess(RECOELE_gsftrack_expected_inner_hits))[i]<=1  // nto used anymore
...
//	  iL_loose_e[N_loose_e]=i;
	...
	if( ((safeAccess(RECOMU_isPFMu))[i] || ((safeAccess(RECOMU_isTrackerHighPtMu))[i] && (safeAccess(RECOMU_PT))[i] > 200.))
      ...
      for( int i = 0; i < RECO_NELE; ++i ){
        ...
        if( debug ) cout << "\n Electron i="<< i <<" properties: "
			 ...
			 << "\n chHad= " <<  (safeAccess(RECOELE_PFchHad))[i]                       
			 << "\n neuHad= " << (safeAccess(RECOELE_PFneuHad))[i]
			 << "\n photon= " <<  (safeAccess(RECOELE_PFphoton))[i]
                         << "\n PU= " << (safeAccess(RECOELE_PFPUchAllPart))[i]
			 << "\nfabs( (safeAccess(RECOELE_SIP))[i] ) " << fabs( (safeAccess(RECOELE_SIP))[i] )
			 << "\nRECOELE_mvaNonTrigV0[i] " << (safeAccess(RECOELE_mvaNonTrigV0))[i]
			 << "\nfabs( (safeAccess(RECOELE_gsftrack_dxy))[i] ) " << fabs( (safeAccess(RECOELE_gsftrack_dxy))[i] )
			 << "\nfabs( (safeAccess(RECOELE_gsftrack_dz))[i] ) " << fabs( (safeAccess(RECOELE_gsftrack_dz))[i] )
			 <<"\n charge = "<<(safeAccess(RECOELE_CHARGE))[i]
			 <<"\n Energy = "<<(safeAccess(RECOELE_E))[i]
			 << endl ;
	...
	hDzEle_tight->Fill( (safeAccess(RECOELE_gsftrack_dz))[i],newweight );
      ...
      auto RECOMU_PFX_dB_new=*RECOMU_PFX_dB;
      auto RECOELE_PFX_rho_new=*RECOELE_PFX_rho;
     /* for (int i=0;i<100;i++){
	...
	for(int e = 0; e < N_loose_e; ++e){
	  ...
	  if( deltaR<=0.3 && ((safeAccess(RECOELE_scl_Eta))[iL_loose_e[e]]< 1.479 || deltaR>0.08) ){ // 0.4 is the isolation cone for electrons in 74x -> 0.3 in 76x              
	...
	float ptFSR;
      ...
      for (int i=0; i<RECOELE_PT->size(); i++){
	/*cout<<"Electron properties PT= "<<(safeAccess(RECOELE_PT))[ iLe[i] ]
	    <<"\n SIP = "<<(safeAccess(RECOELE_SIP))[iLe[i]]
	    <<"\n ISO = "<<RECOELE_PFX_rho_new[iLe[i]]<<endl;*/ }
      
      for(int i = 0; i < Ne_good; ++i){
        for(int j = i + 1; j < Ne_good; ++j){

	  if (fabs((safeAccess(RECOELE_SIP))[iLe[i]])>=4.) continue;
	  ...
	  //cout << "Pairing electrons with pT= " << (safeAccess(RECOELE_PT))[ iLe[i] ] << " and " <<  (safeAccess(RECOELE_PT))[ iLe[j] ] << endl;
	  ...
	  if( has_FSR_Z ){ // if Z has FSR
	    ...
	    /*if( debug ) { cout  << "Z Isolation: "  << std::endl;
      ...
      //Fix memory leak
      candidateZ Z;
      if(std::max(iLe[i],iLe[j]) >= RECOELE_PT->size() or std::min(iLe[i],iLe[j]) < 0) {
            std::cout << "WARNING: bounds errors.\n";
            continue;
        }
	  Z.massvalue=massZ;
	  Z.ilept1=iLe[i];
...
/*
  ...
  */   
      ...
      int ipt1_3;
      int ipt2_3;
      if( (safeAccess(RECOELE_PT))[ iLe[i1] ] > (safeAccess(RECOELE_PT))[ iLe[j1] ] ){ ipt1_3 = iLe[i1]; ipt2_3  = iLe[j1]; }
      else { ipt1_3 = iLe[j1]; ipt2_3  = iLe[i1]; }
      
      hPtLep1_3->Fill( (safeAccess(RECOELE_PT))[ ipt1_3 ],newweight );
      hYLep1_3->Fill( (safeAccess(RECOELE_ETA))[ ipt1_3 ],newweight );
      hIsoLep1_3->Fill( (safeAccess(RECOELE_PFX_rho))[ ipt1_3 ],newweight );
      hSipLep1_3->Fill( (safeAccess(RECOELE_SIP))[ ipt1_3 ],newweight );
      hIpLep1_3->Fill( (safeAccess(RECOELE_IP))[ ipt1_3 ],newweight );
      hIpErLep1_3->Fill( (safeAccess(RECOELE_IPERROR))[ ipt1_3 ],newweight );
      
      
      hPtLep2_3->Fill( (safeAccess(RECOELE_PT))[ ipt2_3 ],newweight );
      hYLep2_3->Fill( (safeAccess(RECOELE_ETA))[ ipt2_3 ],newweight );
      hIsoLep2_3->Fill( (safeAccess(RECOELE_PFX_rho))[ ipt2_3 ],newweight );
      hSipLep2_3->Fill( (safeAccess(RECOELE_SIP))[ ipt2_3 ],newweight );
      hIpLep2_3->Fill( (safeAccess(RECOELE_IP))[ ipt2_3 ],newweight );
      hIpErLep2_3->Fill( (safeAccess(RECOELE_IPERROR))[ ipt2_3 ],newweight );
	...
	if (i==iLe[i1] || i==iLe[j1]) continue;
	 if( (safeAccess(RECOELE_PT))[i] > 7. 
            && fabs((safeAccess(RECOELE_ETA))[i]) < 2.5 
//            && (safeAccess(RECOELE_gsftrack_expected_inner_hits))[i]<=1 
            && fabs((safeAccess(RECOELE_gsftrack_dxy))[i]) < .5 
            && fabs((safeAccess(RECOELE_gsftrack_dz))[i]) < 1. 
            ) {   
	  if( (safeAccess(RECOELE_PFX_rho))[i] > max_Iso_3 ) max_Iso_3 = (safeAccess(RECOELE_PFX_rho))[i] ;
	  if( fabs( (safeAccess(RECOELE_SIP))[i] ) > max_Sip_3 ) max_Sip_3 = fabs( (safeAccess(RECOELE_SIP))[i] ) ;
	  if( fabs( (safeAccess(RECOELE_IP))[i] ) > max_Ip_3 ) max_Ip_3 = fabs( (safeAccess(RECOELE_IP))[i] ) ;
	}
      }
	...
	if((safeAccess(RECO_PFJET_PUID))[i]==1 && (safeAccess(RECO_PFJET_PT))[i]>30. && fabs((safeAccess(RECO_PFJET_ETA))[i])<4.7 ){
	  //Check that jet has deltaR>0.4 away from any tight lepton 
	   ...
	   for(int mu = 0; mu < N_good; ++mu){
            ...
            if ((safeAccess(RECOMU_PFX_dB))[iL[mu]]>=0.35) continue;
	...
	for (int i=0;i<4;i++){ 
	  for (int j=i+1;j<4;j++){
	    ...
	    if ( (safeAccess(RECOELE_CHARGE))[ileptonsee.at(i)] == (safeAccess(RECOELE_CHARGE))[ileptonsee.at(j)] ) continue; // shoud be OS	  
	    // evaluate the mass
	    double mass;
	    
	    Lepton1qcd.SetPtEtaPhiM((safeAccess(RECOELE_PT))[ileptonsee.at(i)], (safeAccess(RECOELE_ETA))[ileptonsee.at(i)], (safeAccess(RECOELE_PHI))[ileptonsee.at(i)], 0.000511);
	...
	for (int ll=0;ll<vindexZZ.size();ll++){
      ...
      //cout << "PTs= " << (safeAccess(RECOELE_PT))[indexlep1Z1] << " " << (safeAccess(RECOELE_PT))[indexlep2Z1] << " " <<  (safeAccess(RECOELE_PT))[indexlep1Z2] << " " << (safeAccess(RECOELE_PT))[indexlep2Z2]<< endl; 
	...
	N_4c_w=N_4c_w+newweight;
      ...
      for(int i = 0; i < 4; ++i){
        Double_t Pt = (safeAccess(RECOELE_PT))[ iL_4L[i] ]; 
        Double_t Eta = (safeAccess(RECOELE_ETA))[ iL_4L[i] ]; 
	Double_t SC_Eta = (safeAccess(RECOELE_scl_Eta))[ iL_4L[i] ]; //Reham

	Double_t Tmp_Pt = Pt;
        
	if(  /*MC_type == "Fall17"  &&*/ DATA_type == "NO"){ 
	      eff_weight*=scale_factors_ele.get_scale_factor(SC_Eta,Pt,!!safeAccess(RECOELE_isGap)[iL_4L[i]]);
        }
      }//end for loop 
	...
	if(  /*MC_type == "Fall17"  &&*/ DATA_type == "NO"){ 
      ...
      //cout << "PTs= " << (safeAccess(RECOELE_PT))[indexleptonfinal[0]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinal[1]] << " " <<  (safeAccess(RECOELE_PT))[indexleptonfinal[2]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinal[3]]<< endl;

      for(int i = 0; i < 4; ++i) tmp_pt[i] =  (safeAccess(RECOELE_PT))[indexleptonfinal[i]];
      for(int i = 0; i < 4; ++i){
        double tmp_max_pt = 0;
      	int jj = i;
        for(int j = 0; j < 4; ++j){
      ...
      /*if(debug)
	  for(int i = 0; i < 4; ++i)
	  cout << "\n ipt[" << i << "] = " << ipt[i] << "\t pt = " << (safeAccess(RECOELE_PT))[ ipt[i] ] << endl; 
      //end sorting*/
	...
	output_txt  << leptformat << endl;
       ...
       if( i == 0){
	 hPtLep1_5->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep1_5->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep1_5->Fill( (safeAccess(RECOELE_PFX_dB))[ ipt[i] ],newweight ) ;
	 hSipLep1_5->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep1_5->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep1_5->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 1){
	 hPtLep2_5->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep2_5->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep2_5->Fill( (safeAccess(RECOELE_PFX_dB))[ ipt[i] ],newweight ) ;
	 hSipLep2_5->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep2_5->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep2_5->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 2){
	 hPtLep3_5->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep3_5->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep3_5->Fill( (safeAccess(RECOELE_PFX_dB))[ ipt[i] ],newweight ) ;
	 hSipLep3_5->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep3_5->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep3_5->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 3){
	 hPtLep4_5->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep4_5->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep4_5->Fill( (safeAccess(RECOELE_PFX_dB))[ ipt[i] ],newweight ) ;
	 hSipLep4_5->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep4_5->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep4_5->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
	...
	if (flagFSR==1){
	  ...
	  Lept.SetPtEtaPhiM((safeAccess(RECOELE_PT))[iLe[i]], (safeAccess(RECOELE_ETA))[iLe[i]], (safeAccess(RECOELE_PHI))[iLe[i]],0.000511);
      ...
      //cout << "PTs= " << (safeAccess(RECOELE_PT))[indexleptonfinalcorr[0]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinalcorr[1]] << " " <<  (safeAccess(RECOELE_PT))[indexleptonfinalcorr[2]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinalcorr[3]]<< endl;

      for(int i = 0; i < 4; ++i) tmp_ptcorr[i] =  (safeAccess(RECOELE_PT))[indexleptonfinalcorr[i]];
      for(int i = 0; i < 4; ++i){
      ...
      if(debug)
      	  for(int i = 0; i < 4; ++i)
      	  	//cout << "\n iptcorr[" << i << "] = " << iptcorr[i] << "\t pt = " << (safeAccess(RECOELE_PT))[ iptcorr[i] ] << endl; 
	...
	TLorentzVector tlv; tlv.SetPtEtaPhiM((safeAccess(RECOELE_PT))[indexleptonfinal[i]], (safeAccess(RECOELE_ETA))[indexleptonfinal[i]], (safeAccess(RECOELE_PHI))[indexleptonfinal[i]], 0.000511); 
	vtlv.push_back(tlv);
        double pterr = (((safeAccess(RECOELE_PT))[]/(safeAccess(RECOELE_P))[indexleptonfinal[i]])*(safeAccess(RECOELE_PTError))[indexleptonfinal[i]]);
	vpterr.push_back(pterr);

	Double_t Pt = (safeAccess(RECOELE_PT))[indexleptonfinal[i]]; 
        Double_t Eta = (safeAccess(RECOELE_ETA))[indexleptonfinal[i]]; 
        int PDGID = 11;
	double corr=1.;
	...
	if(fabs(PDGID) == 11){    
 	  if( MC_type == "Fall11" && DATA_type == "NO"){
 	    ...
 	    if (ebe_ele_mc42x->GetBinContent(binx,biny)>0.) corr=ebe_ele_mc42x->GetBinContent(binx,biny);
            vpterrcorr.push_back(double(((safeAccess(RECOELE_PT))[indexleptonfinal[i]]/(safeAccess(RECOELE_P))[iL_4L[i]])*(safeAccess(RECOELE_PTError))[indexleptonfinal[i]]*corr));        	    
 	  }
	    ...
	    if (ebe_ele_mc53x->GetBinContent(binx,biny)>0.) corr=ebe_ele_mc53x->GetBinContent(binx,biny);
            vpterrcorr.push_back(double(((safeAccess(RECOELE_PT))[indexleptonfinal[i]]/(safeAccess(RECOELE_P))[indexleptonfinal[i]])*(safeAccess(RECOELE_PTError))[indexleptonfinal[i]]*corr));
	  }
...
//       }
     ...
     for(int i = 0; i < 4; ++i){
       ...
       if( i == 0){
	 hPtLep1_7->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep1_7->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep1_7->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep1_7->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep1_7->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep1_7->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 1){
	 hPtLep2_7->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep2_7->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep2_7->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep2_7->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep2_7->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep2_7->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 2){
	 hPtLep3_7->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep3_7->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep3_7->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep3_7->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep3_7->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep3_7->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 3){
	 hPtLep4_7->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep4_7->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep4_7->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep4_7->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep4_7->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep4_7->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
     ...
     //output_txt << "EVENT_7:"<< Run<< ":" << LumiSection<< ":" << Event	<< ":" << mass4l<< ":" << massZ1<< ":" << massZ2 << ":" <<(safeAccess(RECOELE_PT))[ ipt[0] ]<< ":" <<(safeAccess(RECOELE_PT))[ ipt[1] ]<< ":" <<(safeAccess(RECOELE_PT))[ ipt[2] ]<<":"<<(safeAccess(RECOELE_PT))[ ipt[3] ]<<":"<< f_D_bkg_kin<<endl;
     output_txt << "EVENT_7:"<< Run<< ":" << LumiSection<< ":" << Event << ":" << mass4l<< ":" << massZ1<< ":" << massZ2 << ":" <<f_D_bkg_kin<< ":" <<(safeAccess(RECOELE_PT))[ ipt[0] ]<< ":" <<(safeAccess(RECOELE_PT))[ ipt[1] ]<< ":" <<(safeAccess(RECOELE_PT))[ ipt[2] ]<< ":" <<(safeAccess(RECOELE_PT))[ ipt[3] ]<< ":" <<(safeAccess(RECOELE_ETA))[ ipt[0] ]<< ":" <<(safeAccess(RECOELE_ETA))[ ipt[1] ]<< ":" <<(safeAccess(RECOELE_ETA))[ ipt[2] ]<< ":" <<(safeAccess(RECOELE_ETA))[ ipt[3] ]<< ":" <<(safeAccess(RECOELE_PHI))[ ipt[0] ]<< ":" <<(safeAccess(RECOELE_PHI))[ ipt[1] ]<< ":" <<(safeAccess(RECOELE_PHI))[ ipt[2] ]<< ":" <<(safeAccess(RECOELE_PHI))[ ipt[3] ]<<endl;
     ...
     for(int i = 0; i < 4; ++i){
       ...
       if( i == 0){
	 hPtLep1_8->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep1_8->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep1_8->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep1_8->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep1_8->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep1_8->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 1){
	 hPtLep2_8->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep2_8->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep2_8->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep2_8->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep2_8->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep2_8->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 2){
	 hPtLep3_8->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep3_8->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep3_8->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep3_8->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep3_8->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep3_8->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
       if( i == 3){
	 hPtLep4_8->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;
	 hYLep4_8->Fill(  (safeAccess(RECOELE_ETA))[ ipt[i] ],newweight ) ;
	 hIsoLep4_8->Fill( RECOELE_PFX_rho_new[ ipt[i] ],newweight ) ;
	 hSipLep4_8->Fill( (safeAccess(RECOELE_SIP))[ ipt[i] ],newweight ) ;
	 hIpLep4_8->Fill( (safeAccess(RECOELE_IP))[ ipt[i] ],newweight ) ;
	 hIpErLep4_8->Fill( (safeAccess(RECOELE_IPERROR))[ ipt[i] ],newweight ) ;
       }
...
//	 if((safeAccess(cSV_BTagJet_PT))[i]>30. && fabs((safeAccess(cSV_BTagJet_ETA))[i])<4.7 ) cout << "Found a bjet (pT>30 and |eta|<2.4) with pT= " << (safeAccess(cSV_BTagJet_PT))[i] << endl;	 
     ...
     f_lept4_pt_error = (safeAccess(RECOELE_PTError))[iptcorr[3]] ;
     ...
     /*cout << "Final: pT of final leptons from Z1 and Z2 ="
	  << (safeAccess(RECOELE_PT))[indexleptonfinal[0]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinal[1]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinal[2]] << " " << (safeAccess(RECOELE_PT))[indexleptonfinal[3]] <<endl;*/
     ...
     // 	   cout << "Muon with pT= " << (safeAccess(RECOELE_PT))[indexleptonfinal[i]] << " has associated a photon with pT= " << (safeAccess(RECOPFPHOT_PT))[iLp[p]] <<  endl;
     // 	   flagFSR=1;
     // 	   pfsr=p;
     // 	 }
     //   }
	 ...
	 partP.SetPtEtaPhiE(L11P4.Pt()+(safeAccess(RECOELE_PTError))[indexleptonfinal[0]],L11P4.Eta(),L11P4.Phi(),L11P4.E());		vpartP.push_back( partP );//+ Pt Uncertainty
	 partP.SetPtEtaPhiE(L11P4.Pt()-(safeAccess(RECOELE_PTError))[indexleptonfinal[0]],L11P4.Eta(),L11P4.Phi(),L11P4.E());		vpartP.push_back( partP );//- Pt Uncertainty
	 vpartsP.push_back( vpartP );

	 vpartP.clear();
	 partP.SetPtEtaPhiE(L12P4.Pt(),L12P4.Eta(),L12P4.Phi(),L12P4.E());						vpartP.push_back( partP );//Nominal
	 partP.SetPtEtaPhiE(L12P4.Pt()+(safeAccess(RECOELE_PTError))[indexleptonfinal[1]],L12P4.Eta(),L12P4.Phi(),L12P4.E());		vpartP.push_back( partP );//+Pt Uncertainty
	 partP.SetPtEtaPhiE(L12P4.Pt()-(safeAccess(RECOELE_PTError))[indexleptonfinal[1]],L12P4.Eta(),L12P4.Phi(),L12P4.E());		vpartP.push_back( partP );//-Pt Uncertainty
	 vpartsP.push_back( vpartP );

	 vpartP.clear();
	 partP.SetPtEtaPhiE(L21P4.Pt(),L21P4.Eta(),L21P4.Phi(),L21P4.E());						vpartP.push_back( partP );//Nominal
	 partP.SetPtEtaPhiE(L21P4.Pt()+(safeAccess(RECOELE_PTError))[indexleptonfinal[2]],L21P4.Eta(),L21P4.Phi(),L21P4.E());		vpartP.push_back( partP );//+Pt Uncertainty
	 partP.SetPtEtaPhiE(L21P4.Pt()-(safeAccess(RECOELE_PTError))[indexleptonfinal[2]],L21P4.Eta(),L21P4.Phi(),L21P4.E());		vpartP.push_back( partP );//-Pt Uncertainty
	 vpartsP.push_back( vpartP );
	 
	 vpartP.clear();
	 partP.SetPtEtaPhiE(L22P4.Pt(),L22P4.Eta(),L22P4.Phi(),L22P4.E());						vpartP.push_back( partP );//Nominal
	 partP.SetPtEtaPhiE(L22P4.Pt()+(safeAccess(RECOELE_PTError))[indexleptonfinal[3]],L22P4.Eta(),L22P4.Phi(),L22P4.E());		vpartP.push_back( partP );//+Pt Uncertainty
	 partP.SetPtEtaPhiE(L22P4.Pt()-(safeAccess(RECOELE_PTError))[indexleptonfinal[3]],L22P4.Eta(),L22P4.Phi(),L22P4.E());		vpartP.push_back( partP );//-Pt Uncertainty
	 vpartsP.push_back( vpartP );
     ...
     if (abs(mass4l-125.)<=10. && Ne_good==4 && n_bjets<=1  && pass_METFilters) {
       ++N_10;
	 ...
	 if( i == 0){
	   hPtLep1_10->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;	   
	 }
	 if( i == 1){
	   hPtLep2_10->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;	  
	 }
	 if( i == 2){
	   hPtLep3_10->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;	   
	 }
	 if( i == 3){
	   hPtLep4_10->Fill( (safeAccess(RECOELE_PT))[ ipt[i] ],newweight ) ;	   
	 }
     ...
     if (abs(mass4l-125.)<=10. && Ne_good==4 && n_bjets<=1 && RECO_PFMET>=50. && pass_METFilters) {
       ++N_11;
     ...
     if (debug) for(unsigned int i=0; i< 4; i++) output_txt << "\t mu[" << i << "] pT: " << (safeAccess(RECOELE_PT))[indexleptonfinal[i]] << "\t eta: " <<  (safeAccess(RECOELE_ETA))[indexleptonfinal[i]] << "\t phi: " <<  (safeAccess(RECOELE_PHI))[indexleptonfinal[i]] << endl;
   ...
   // If there are no good entries, print a warning and exit normally
   if (good_events==0) {
        std::cerr << "No events in good lumi sections.\n";
        std::cout << "No events in good lumi sections.\n";
        exit(0);
   } else {
        std::cerr << "Good events: " << good_events << " Bad events: " << bad_events << "\n";
        std::cout << "Good events: " << good_events << " Bad events: " << bad_events << "\n";
   }    
