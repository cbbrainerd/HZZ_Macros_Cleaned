...
while [ $n -lt ${nlines} ]; do
  ...
  head -n1 data_input.txt > DataCards$4$2/data_input_${n}.txt
  samplename=`awk '{print$1}' DataCards$4$2/data_input_${n}.txt`
  echo $samplename
  tail -n $m data_input.txt > data_input_tmp.txt
  mv  data_input_tmp.txt data_input.txt
  rm -f jobs4mu/submit_ReferenceAnalysis_${samplename}_4mu.sh
  SITE="$1" YEAR="$2" MC="$3" HISTODIR=
  if [ $1 = ${SCERN} ]; then
