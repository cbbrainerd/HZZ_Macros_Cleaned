...
*/
      ...
      if( datasetName.Contains("amcatnlo")) {
        std::cout << "Reweighting sample of amcatnlo with weight= " << MC_weighting << endl;
      ...
      if (isSignal || isggH){
        if( ( safeAccess(MC_PDGID)[3] == 13 && safeAccess(MC_PDGID)[4] == -13  &&  safeAccess(MC_PDGID)[5] == 11 && safeAccess(MC_PDGID)[6] == -11 )
            || ( safeAccess(MC_PDGID)[3] == 11 && safeAccess(MC_PDGID)[4] == -11  &&  safeAccess(MC_PDGID)[5] == 13 && safeAccess(MC_PDGID)[6] == -13 )){
          //if( ( safeAccess(MC_PDGID)[3] == 13 && safeAccess(MC_PDGID)[4] == -13  &&  safeAccess(MC_PDGID)[5] == 15 && safeAccess(MC_PDGID)[6] == -15 ) // 2l2tau
          //|| ( safeAccess(MC_PDGID)[3] == 11 && safeAccess(MC_PDGID)[4] == -11  &&  safeAccess(MC_PDGID)[5] == 15 && safeAccess(MC_PDGID)[6] == -15 ) 
          //|| ( safeAccess(MC_PDGID)[3] == 15 && safeAccess(MC_PDGID)[4] == -15  &&  safeAccess(MC_PDGID)[5] == 11 && safeAccess(MC_PDGID)[6] == -11 )
          //|| ( safeAccess(MC_PDGID)[3] == 15 && safeAccess(MC_PDGID)[4] == -15  &&  safeAccess(MC_PDGID)[5] == 13 && safeAccess(MC_PDGID)[6] == -13 )
          //|| ( safeAccess(MC_PDGID)[3] == 15 && safeAccess(MC_PDGID)[4] == -15  &&  safeAccess(MC_PDGID)[5] == 15 && safeAccess(MC_PDGID)[6] == -15 )
          //){
          is2e2mu=true;
          ++N_01 ;  // fill counter                                                                                                                                             
	...
	for( int i = 3; i  < 7; ++i){
	  ...
	  //if( fabs(safeAccess(MC_PDGID)[i]) == 13 && ( fabs(safeAccess(MC_ETA)[i]) >= 2.4 || safeAccess(MC_PT)[i] <= 5. ) ){
          if( fabs(safeAccess(MC_PDGID)[i]) == 13 && ( fabs(safeAccess(MC_ETA)[i]) >= 2.4) ){
            isInAcceptance = 0;
            break;        
          }
          //if( fabs(safeAccess(MC_PDGID)[i]) == 11 && ( fabs(safeAccess(MC_ETA)[i]) >= 2.5 || safeAccess(MC_PT)[i] <= 7. ) ){
          if( fabs(safeAccess(MC_PDGID)[i]) == 11 && ( fabs(safeAccess(MC_ETA)[i]) >= 2.5) ){
	...
	if ( safeAccess(RECOPFPHOT_PT)[i] > 2. && fabs(safeAccess(RECOPFPHOT_ETA)[i]) < 2.4 && safeAccess(RECOPFPHOT_PFX_rho)[i]<1.8) {
	  ...
	  for(int e = 0; e < N_loose_e; ++e){
	    ...
	    cout << "debug: " << safeAccess(RECOELE_PT)[iL_loose_e[e]] << " " << deltaPhi << " " << deltaEta << " " << deltaR << endl;
	...
	for(int l = 0; l < N_loose_mu; ++l){ // loop on muons
	  ...
	  cout << "DeltaR= " << deltaR << " " <<  deltaR/pow(safeAccess(RECOPFPHOT_PT)[iLp[i]],2) << endl;
	...
	if (iLp_l[i]!=-1 && iLp_tagEM[i]==1) cout << "There is photon with pT= " << safeAccess(RECOPFPHOT_PT)[iLp[i]] << " attached to an electron with pT= " << safeAccess(RECOELE_PT)[iLp_l[i]] << endl;
      };
	...
	for(int e = 0; e < N_loose_e; ++e){
	  ...
	  double deltaR = sqrt( pow( DELTAPHI( safeAccess(RECOPFPHOT_PHI)[iLp[i]] , safeAccess(RECOELE_PHI)[iL_loose_e[e]] ),2) + pow(safeAccess(RECOPFPHOT_ETA)[iLp[i]] - safeAccess(RECOELE_ETA)[iL_loose_e[e]],2) );
	...
	int tag;
	  ...
	  if (fabs(safeAccess(RECOMU_SIP)[iL[i]])>=4.) continue; // SIP cut
	  ...
	  cout << "\n Pairing muons with pT= " << safeAccess(RECOMU_PT)[ iL[i] ] << " and " <<  safeAccess(RECOMU_PT)[ iL[j] ] << endl;
	  ...
	  if (fabs(safeAccess(RECOELE_SIP)[iLe[i]])>=4.) continue; // SIP cut
	  if (fabs(safeAccess(RECOELE_SIP)[iLe[j]])>=4.) continue;
	  // if (fabs(RECOELE_PFX_rho_new[iLe[i]])>=0.35) continue; // Isolation cut
	  // if (fabs(RECOELE_PFX_rho_new[iLe[j]])>=0.35) continue;
	  
	  if(safeAccess(RECOELE_CHARGE)[ iLe[j] ] == safeAccess(RECOELE_CHARGE)[ iLe[i] ]) continue; // opposite charge

	  cout << "\n Pairing electrons with pT= " << safeAccess(RECOELE_PT)[ iLe[i] ] << " and " <<  safeAccess(RECOELE_PT)[ iLe[j] ] << endl;
	  ...
	  float pTphot=-999.;
	  Lepton1.SetPtEtaPhiM(safeAccess(RECOELE_PT)[iLe[i]], safeAccess(RECOELE_ETA)[iLe[i]], safeAccess(RECOELE_PHI)[iLe[i]], 0.000511);
	  Lepton2.SetPtEtaPhiM(safeAccess(RECOELE_PT)[iLe[j]], safeAccess(RECOELE_ETA)[iLe[j]], safeAccess(RECOELE_PHI)[iLe[j]], 0.000511);
	  DiLepton=Lepton1+Lepton2;	  
	  massZ = DiLepton.M();	  
	  massZ_noFSR = massZ;
	  if (debug) cout << "Mass Z= " << massZ << endl;
	  ...
	  if( has_FSR_Z ){ // if Z has FSR
	    ...
	    // do not recompute isolation here
	    if( debug ) cout  << "Z Isolation (not corrected for photon): "
                              << "\n safeAccess(RECOELE_PFX_rho)[ iLe[i] ] " << safeAccess(RECOELE_PFX_rho)[ iLe[i] ]
                              << "\n safeAccess(RECOELE_PFX_rho)[ iLe[j] ] " << safeAccess(RECOELE_PFX_rho)[ iLe[j] ]
                              << endl;
	    
	    if( pi != -1 ){
	      //double deltaR_i = sqrt( pow( DELTAPHI( safeAccess(RECOPFPHOT_PHI)[iLp[pi]] , safeAccess(RECOELE_PHI)[iLe[i]] ),2) + pow(safeAccess(RECOPFPHOT_ETA)[iLp[pi]] - safeAccess(RECOELE_ETA)[iLe[i]],2) );
              //if( deltaR_i < 0.4 && deltaR_i > 0.01 )
	      pTphot=safeAccess(RECOPFPHOT_PT)[iLp[pi]];	      
	    }
	    else if( pj != -1 ){	      
              //double deltaR_j = sqrt( pow( DELTAPHI( safeAccess(RECOPFPHOT_PHI)[iLp[pj]] , safeAccess(RECOELE_PHI)[iLe[j]] ),2) + pow(safeAccess(RECOPFPHOT_ETA)[iLp[pj]] - safeAccess(RECOELE_ETA)[iLe[j]],2) );
              //if( deltaR_j < 0.4 && deltaR_j > 0.01 )
	      pTphot=safeAccess(RECOPFPHOT_PT)[iLp[pj]];	      
	    }
	  ...
	  Z->ilept1=iLe[i];
	  Z->ilept2=iLe[j];
	  Z->pt1=safeAccess(RECOELE_PT)[iLe[i]];
	  Z->pt2=safeAccess(RECOELE_PT)[iLe[j]];
	  Z->eta1=safeAccess(RECOELE_ETA)[iLe[i]];
	  Z->eta2=safeAccess(RECOELE_ETA)[iLe[j]];
	  Z->phi1=safeAccess(RECOELE_PHI)[iLe[i]];
	  Z->phi2=safeAccess(RECOELE_PHI)[iLe[j]];
	  Z->charge1=safeAccess(RECOELE_CHARGE)[iLe[i]];
	  Z->charge2=safeAccess(RECOELE_CHARGE)[iLe[j]];
	...
	continue;
      ...
      for (int index=0; index<Zcandisolmassvector.size();index++){
	    ...
	    if(  MC_type == "Fall17"  && DATA_type == "NO"){
          ...
          for(int i = 0; i < 2; ++i){
            Double_t Pt = safeAccess(RECOELE_PT)[ z1lept[i] ]; 
	...
	//add Reco efficiency
      ...
      int ipt1_3;
      int ipt2_3;
      if( safeAccess(RECOMU_PT)[ iL[i1] ] > safeAccess(RECOMU_PT)[ iL[j1] ] ){ ipt1_3 = iL[i1]; ipt2_3  = iL[j1]; }
      else { ipt1_3 = iL[j1]; ipt2_3  = iL[i1]; }
      
      hPtLep1_3->Fill( safeAccess(RECOMU_PT)[ ipt1_3 ],newweight );
      hYLep1_3->Fill( safeAccess(RECOMU_ETA)[ ipt1_3 ],newweight );
      hIsoLep1_3->Fill( safeAccess(RECOMU_PFX_dB)[ ipt1_3 ],newweight );
      hSipLep1_3->Fill( safeAccess(RECOMU_SIP)[ ipt1_3 ],newweight );
      hIpLep1_3->Fill( safeAccess(RECOMU_IP)[ ipt1_3 ],newweight );
      hIpErLep1_3->Fill( safeAccess(RECOMU_IPERROR)[ ipt1_3 ],newweight );
      
      
      hPtLep2_3->Fill( safeAccess(RECOMU_PT)[ ipt2_3 ],newweight );
      hYLep2_3->Fill( safeAccess(RECOMU_ETA)[ ipt2_3 ],newweight );
      hIsoLep2_3->Fill( safeAccess(RECOMU_PFX_dB)[ ipt2_3 ],newweight );
      hSipLep2_3->Fill( safeAccess(RECOMU_SIP)[ ipt2_3 ],newweight );
      hIpLep2_3->Fill( safeAccess(RECOMU_IP)[ ipt2_3 ],newweight );
      hIpErLep2_3->Fill( safeAccess(RECOMU_IPERROR)[ ipt2_3 ],newweight );
	...
	Lepton1qcd.SetPtEtaPhiM(safeAccess(RECOMU_PT)[ileptonsmumu.at(0)], safeAccess(RECOMU_ETA)[ileptonsmumu.at(0)], safeAccess(RECOMU_PHI)[ileptonsmumu.at(0)], 0.105);
	Lepton2qcd.SetPtEtaPhiM(safeAccess(RECOMU_PT)[ileptonsmumu.at(1)], safeAccess(RECOMU_ETA)[ileptonsmumu.at(1)], safeAccess(RECOMU_PHI)[ileptonsmumu.at(1)], 0.105);
	...
	// ee	
	cout << "Checking ee pairs for QCD rejection" << endl;       
	Lepton1qcd.SetPtEtaPhiM(safeAccess(RECOELE_PT)[ileptonsee.at(0)], safeAccess(RECOELE_ETA)[ileptonsee.at(0)], safeAccess(RECOELE_PHI)[ileptonsee.at(0)], 0.000511);
	Lepton2qcd.SetPtEtaPhiM(safeAccess(RECOELE_PT)[ileptonsee.at(1)], safeAccess(RECOELE_ETA)[ileptonsee.at(1)], safeAccess(RECOELE_PHI)[ileptonsee.at(1)], 0.000511);
	DiLeptonQCD=Lepton1qcd+Lepton2qcd;       
	mass = DiLeptonQCD.M();
	...
	for (int i=0;i<2;i++){ 
	  for (int j=0;j<2;j++){
	    ...
	    if ( safeAccess(RECOMU_CHARGE)[ileptonsmumu.at(i)] == safeAccess(RECOELE_CHARGE)[ileptonsee.at(j)] ) continue; // shoud be OS	  
	    // evaluate the mass
	    double mass;
	    
	    Lepton1qcd.SetPtEtaPhiM(safeAccess(RECOMU_PT)[ileptonsmumu.at(i)], safeAccess(RECOMU_ETA)[ileptonsmumu.at(i)], safeAccess(RECOMU_PHI)[ileptonsmumu.at(i)], 0.105);
	    Lepton2qcd.SetPtEtaPhiM(safeAccess(RECOELE_PT)[ileptonsee.at(j)], safeAccess(RECOELE_ETA)[ileptonsee.at(j)], safeAccess(RECOELE_PHI)[ileptonsee.at(j)], 0.000511);
	    DiLeptonQCD=Lepton1qcd+Lepton2qcd;       
	    mass = DiLeptonQCD.M();
	...
	for (int ll=0;ll<vindexZZ.size();ll++){
      ...
      if (Z1tag==1) cout << "PTs= " << safeAccess(RECOMU_PT)[indexlep1Z1] << " " << safeAccess(RECOMU_PT)[indexlep2Z1] << " " <<  safeAccess(RECOELE_PT)[indexlep1Z2] << " " << safeAccess(RECOELE_PT)[indexlep2Z2]<< endl; 
      if (Z1tag==2) cout << "PTs= " << safeAccess(RECOELE_PT)[indexlep1Z1] << " " << safeAccess(RECOELE_PT)[indexlep2Z1] << " " <<  safeAccess(RECOMU_PT)[indexlep1Z2] << " " << safeAccess(RECOMU_PT)[indexlep2Z2]<< endl; 
	...
	if(  MC_type == "Fall17"  && DATA_type == "NO"){ 
      ...
      if (Z2tag==1){ 
        for(int i = 0; i < 2; ++i){
          Double_t Pt = safeAccess(RECOMU_PT)[ z2lept[i] ]; 
          Double_t Eta = safeAccess(RECOMU_ETA)[ z2lept[i] ];
	  ...
	  if(  MC_type == "Fall17"  && DATA_type == "NO"){
        ...
        for(int i = 0; i < 2; ++i){
          Double_t Pt = safeAccess(RECOELE_PT)[ z2lept[i] ]; 
	...
	for(int i = 0; i < 2; ++i){
      ...
      for(int i = 0; i < 2; ++i){ 
	if (Z1tag==1) tmp_pt[i] =  safeAccess(RECOMU_PT)[indexleptonfinal[i]];
	if (Z1tag==2) tmp_pt[i] =  safeAccess(RECOELE_PT)[indexleptonfinal[i]];
	cout << tmp_pt[i] << endl;
      }
      for(int i = 2; i < 4; ++i){ 
	if (Z2tag==1) tmp_pt[i] =  safeAccess(RECOMU_PT)[indexleptonfinal[i]];
	if (Z2tag==2) tmp_pt[i] =  safeAccess(RECOELE_PT)[indexleptonfinal[i]];
	cout << tmp_pt[i] << endl;
      }
	...
	for (int j=0;j<RECO_NMU;j++) {	    
	  if (safeAccess(RECOMU_PT)[j]==sortedpT[i]) {
	    ismuon=true;
	    break;
	  }
	  else {
	    for (int j=0;j<RECO_NELE;j++) {	    
	      if (safeAccess(RECOELE_PT)[j]==sortedpT[i]) {
		iselectron=true;
		break;
	      }
	    }
	...
	/*for (int j=0;j<RECO_NELE;j++) {	    
	  if (safeAccess(RECOELE_PT)[j]==sortedpT[i]) iselectron=true;
	}
	*/
	...
	if (ismuon){
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == ipt[i] && iLp_tagEM[ p ] == 0 )  {
	      cout << "Muon with pT= " << safeAccess(RECOMU_PT)[ipt[i]] << " has associated a photon with pT= " << safeAccess(RECOPFPHOT_PT)[iLp[p]] <<  endl;
	      flagFSR_tag=0;
	      pfsr=p;
	      break;
	    }
	  ...
	  for( int p = 0; p < Nphotons; ++p ){
	    if( iLp_l[ p ] == ipt[i] && iLp_tagEM[ p ] == 1 )  {
	      cout << "Electron with pT= " << safeAccess(RECOELE_PT)[ipt[i]] << " has associated a photon with pT= " << safeAccess(RECOPFPHOT_PT)[iLp[p]] <<  endl;
	      flagFSR_tag=1;
	      pfsr=p;
	      break;
	    }
	...
	if (ismuon && flagFSR_tag==0){
	  ...
	  sprintf (leptformat,"FormatLept=%d:%d:%d:%d:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.2f:%.3f:%.3f:%.2f:%.2f:%.2f",
		   Run,LumiSection,Event,
		   int(-11*safeAccess(RECOELE_CHARGE)[ipt[i]]),
		   safeAccess(RECOELE_PT)[ipt[i]],safeAccess(RECOELE_ETA)[ipt[i]],safeAccess(RECOELE_PHI)[ipt[i]],safeAccess(RECOELE_SIP)[ipt[i]],
		   safeAccess(RECOELE_PFchHad)[ipt[i]],safeAccess(RECOELE_PFneuHad)[ipt[i]],safeAccess(RECOELE_PFphoton)[ipt[i]],RHO_ele,safeAccess(RECOELE_PFX_rho)[ipt[i]],safeAccess(RECOELE_mvaNonTrigV0)[ipt[i]],
		   safeAccess(RECOPFPHOT_PT)[iLp[pfsr]],RECOPFPHOT_DR[iLp[pfsr]],safeAccess(RECOPFPHOT_PFX_rho)[iLp[pfsr]]
		   );
	}	
	else if (ismuon){	  	  	      
	...
	for( int p = 0; p < Nphotons; ++p ){
	  ...
	  if( iLp_l[ p ] == iLe[i] && iLp_tagEM[ p ] == 1 )  {
	    cout << "Electron with pT= " << safeAccess(RECOELE_PT)[iLe[i]] << " has associated a photon with pT= " << safeAccess(RECOPFPHOT_PT)[iLp[p]] <<  endl;
	    // RECOELE_PFX_rho_new[iLe[i]]=
	    //   (safeAccess(RECOELE_PFchHad)[iLe[i]]+
	    //    max(0.,safeAccess(RECOELE_PFneuHad)[iLe[i]]+
	    // 	   (safeAccess(RECOELE_PFphoton)[iLe[i]]-safeAccess(RECOPFPHOT_PT)[iLp[p]] )-
	    // 	   max(RHO_ele,0.0)*(EffectiveArea)))/safeAccess(RECOELE_PT)[iLe[i]];	    
	    flagFSR=1;
	    pfsr=p;
	  }
	}
	...
	if (flagFSR==1){
	  cout << "Before correcting for FSR; electron pT= " << safeAccess(RECOELE_PT)[iLe[i]] << " Eta= " << safeAccess(RECOELE_ETA)[iLe[i]] << " Phi= " << safeAccess(RECOELE_PHI)[iLe[i]] << endl;
	  TLorentzVector Lept,LeptCorrection;
	  Lept.SetPtEtaPhiM(safeAccess(RECOELE_PT)[iLe[i]], safeAccess(RECOELE_ETA)[iLe[i]], safeAccess(RECOELE_PHI)[iLe[i]], 0.105);
	  LeptCorrection.SetPtEtaPhiM(safeAccess(RECOPFPHOT_PT)[iLp[pfsr]],safeAccess(RECOPFPHOT_ETA)[iLp[pfsr]],safeAccess(RECOPFPHOT_PHI)[iLp[pfsr]],0);
	  Lept+=LeptCorrection;
	  safeAccess(RECOELE_PT)[iLe[i]]=Lept.Pt();
	  safeAccess(RECOELE_ETA)[iLe[i]]=Lept.Eta();
	  safeAccess(RECOELE_PHI)[iLe[i]]=Lept.Phi();
	  cout << "After correcting for FSR; muon pT= " << safeAccess(RECOELE_PT)[iLe[i]] << " Eta= " << safeAccess(RECOELE_ETA)[iLe[i]] << " Phi= " << safeAccess(RECOELE_PHI)[iLe[i]] << endl;
	}
      }
      ...
      for(int i = 0; i < 2; ++i){ 
	if (Z1tag==1) tmp_ptcorr[i] =  safeAccess(RECOMU_PT)[indexleptonfinalcorr[i]];	
	if (Z1tag==2) tmp_ptcorr[i] =  safeAccess(RECOELE_PT)[indexleptonfinalcorr[i]];
	tmpfl_ptcorr[i] =  Z1tag;
	cout << tmp_ptcorr[i] << endl;
      }
      for(int i = 2; i < 4; ++i){ 
	if (Z2tag==1) tmp_ptcorr[i] =  safeAccess(RECOMU_PT)[indexleptonfinalcorr[i]];
	if (Z2tag==2) tmp_ptcorr[i] =  safeAccess(RECOELE_PT)[indexleptonfinalcorr[i]];
...
//       }
   ...
   output_txt << "EVENT_7:"<< Run<< ":" << LumiSection<< ":" << Event << ":" << mass4l<< ":" << massZ1<< ":" << massZ2 << ":" << f_D_bkg_kin<< ":" << safeAccess(RECOMU_PT)[ ipt[0] ] << ":" << safeAccess(RECOMU_PT)[ ipt[1] ] << ":" << safeAccess(RECOELE_PT)[ ipt[2] ] << ":" << safeAccess(RECOELE_PT)[ ipt[3] ] << ":" << safeAccess(RECOMU_ETA)[ ipt[0] ] << ":" <<safeAccess(RECOMU_ETA)[ ipt[1] ]<< ":" <<safeAccess(RECOELE_ETA)[ ipt[2] ]<< ":" <<safeAccess(RECOELE_ETA)[ ipt[3] ]<< ":" << safeAccess(RECOMU_PHI)[ ipt[0] ] << ":" << safeAccess(RECOMU_PHI)[ ipt[1] ] << ":" << safeAccess(RECOELE_PHI)[ ipt[2] ] << ":" << safeAccess(RECOELE_PHI)[ ipt[3] ]<<endl;
     
} 
     else if (Z1tag==2 && Z2tag==1){// ZeeZmumu

     output_txt << "EVENT_7:"<< Run<< ":" << LumiSection<< ":" << Event << ":" << mass4l<< ":" << massZ1<< ":" << massZ2 << ":" << f_D_bkg_kin<< ":" <<safeAccess(RECOELE_PT)[ ipt[0] ]<< ":" <<safeAccess(RECOELE_PT)[ ipt[1] ]<< ":" <<safeAccess(RECOMU_PT)[ ipt[2] ]<<":"<<safeAccess(RECOMU_PT)[ ipt[3] ]<< ":" <<safeAccess(RECOELE_ETA)[ ipt[0] ]<< ":" <<safeAccess(RECOELE_ETA)[ ipt[1] ]<< ":" <<safeAccess(RECOMU_ETA)[ ipt[2] ]<< ":" <<safeAccess(RECOMU_ETA)[ ipt[3] ]<< ":" << safeAccess(RECOELE_PHI)[ ipt[0] ]<< ":" <<safeAccess(RECOELE_PHI)[ ipt[1] ]<< ":" <<safeAccess(RECOMU_PHI)[ ipt[2] ]<<":" <<safeAccess(RECOMU_PHI)[ ipt[3] ]<<endl;
      }
    ...
    int n_bjets_loose =0;
     ...
     for (int i=0;i<50;i++){
       //if (safeAccess(cSV_BTagJet_DISCR)[i] > 0.460){ // Loose
       if (safeAccess(cSV_BTagJet_DISCR)[i] > 0.4941){ // Medium     
     ...
     if (sortedfl_pTcorr[0]==2){
       f_lept1_pt  = safeAccess(RECOELE_PT)[iptcorr[0]] ;
       f_lept1_pt_error = safeAccess(RECOELE_PTError)[iptcorr[0]] ;
       f_lept1_eta = safeAccess(RECOELE_ETA)[iptcorr[0]] ;
       f_lept1_phi = safeAccess(RECOELE_PHI)[iptcorr[0]];
       f_lept1_charge = safeAccess(RECOELE_CHARGE)[iptcorr[0]];
       f_lept1_pfx = RECOELE_PFX_rho_new[iptcorr[0]];
       f_lept1_sip = safeAccess(RECOELE_SIP)[iptcorr[0]];
       //    f_lept1_mvaid = safeAccess(RECOELE_mvaNonTrigV0)[iptcorr[0]];
     }
     ...
     if (sortedfl_pTcorr[1]==2){
       f_lept2_pt  = safeAccess(RECOELE_PT)[iptcorr[1]] ;
       f_lept2_pt_error = safeAccess(RECOELE_PTError)[iptcorr[1]] ;
       f_lept2_eta = safeAccess(RECOELE_ETA)[iptcorr[1]] ;
       f_lept2_phi = safeAccess(RECOELE_PHI)[iptcorr[1]];
       f_lept2_charge = safeAccess(RECOELE_CHARGE)[iptcorr[1]];
       f_lept2_pfx = RECOELE_PFX_rho_new[iptcorr[1]];
       f_lept2_sip = safeAccess(RECOELE_SIP)[iptcorr[1]];
       //    f_lept2_mvaid = safeAccess(RECOELE_mvaNonTrigV0)[iptcorr[1]];
     }
     ...
     if (sortedfl_pTcorr[2]==2){
       f_lept3_pt  = safeAccess(RECOELE_PT)[iptcorr[2]] ;
       f_lept3_pt_error = safeAccess(RECOELE_PTError)[iptcorr[2]] ;     
       f_lept3_eta = safeAccess(RECOELE_ETA)[iptcorr[2]] ;
       f_lept3_phi = safeAccess(RECOELE_PHI)[iptcorr[2]];
       f_lept3_charge = safeAccess(RECOELE_CHARGE)[iptcorr[2]];
       f_lept3_pfx = RECOELE_PFX_rho_new[iptcorr[2]];
       f_lept3_sip = safeAccess(RECOELE_SIP)[iptcorr[2]];
       //    f_lept3_mvaid = safeAccess(RECOELE_mvaNonTrigV0)[iptcorr[2]];
     }
     ...
     if (sortedfl_pTcorr[3]==2){
       f_lept4_pt  = safeAccess(RECOELE_PT)[iptcorr[3]] ;
       f_lept4_pt_error = safeAccess(RECOMU_mubesttrkPTError)[iptcorr[3]] ;
       f_lept4_eta = safeAccess(RECOELE_ETA)[iptcorr[3]] ;
       f_lept4_phi = safeAccess(RECOELE_PHI)[iptcorr[3]];
       f_lept4_charge = safeAccess(RECOELE_CHARGE)[iptcorr[3]];
       f_lept4_pfx = RECOELE_PFX_rho_new[iptcorr[3]];
       f_lept4_sip = safeAccess(RECOELE_SIP)[iptcorr[3]];
       //    f_lept4_mvaid = safeAccess(RECOELE_mvaNonTrigV0)[iptcorr[3]];
     }
     ...
     if (Z1tag==1) cout << safeAccess(RECOMU_PT)[indexleptonfinal[0]] << " " << safeAccess(RECOMU_PT)[indexleptonfinal[1]] << " " << safeAccess(RECOELE_PT)[indexleptonfinal[2]] << " " << safeAccess(RECOELE_PT)[indexleptonfinal[3]] <<endl;
     if (Z1tag==2) cout << safeAccess(RECOELE_PT)[indexleptonfinal[0]] << " " << safeAccess(RECOELE_PT)[indexleptonfinal[1]] << " " << safeAccess(RECOMU_PT)[indexleptonfinal[2]] << " " << safeAccess(RECOMU_PT)[indexleptonfinal[3]] <<endl;
     ...
     if (Z1tag==1 && Z2tag==2){
       ...
       L21P4.SetPtEtaPhiM(safeAccess(RECOELE_PT)[indexleptonfinal[2]], safeAccess(RECOELE_ETA)[indexleptonfinal[2]], safeAccess(RECOELE_PHI)[indexleptonfinal[2]], 0.000511);
       L22P4.SetPtEtaPhiM(safeAccess(RECOELE_PT)[indexleptonfinal[3]], safeAccess(RECOELE_ETA)[indexleptonfinal[3]], safeAccess(RECOELE_PHI)[indexleptonfinal[3]], 0.000511);
     }

     if (Z1tag==2 && Z2tag==1){
       L11P4.SetPtEtaPhiM(safeAccess(RECOELE_PT)[indexleptonfinal[0]], safeAccess(RECOELE_ETA)[indexleptonfinal[0]], safeAccess(RECOELE_PHI)[indexleptonfinal[0]], 0.000511);
       L12P4.SetPtEtaPhiM(safeAccess(RECOELE_PT)[indexleptonfinal[1]], safeAccess(RECOELE_ETA)[indexleptonfinal[1]], safeAccess(RECOELE_PHI)[indexleptonfinal[1]], 0.000511);
	 ...
	 f_lept2_pdgid=-13;
       ...
       if (safeAccess(RECOELE_CHARGE)[indexleptonfinal[2]] == 1){
	 L21PID=-11;
	 L22PID=11;
	 f_lept3_pdgid=-11;
	 f_lept4_pdgid=11;
       }
       else if (safeAccess(RECOELE_CHARGE)[indexleptonfinal[2]] == -1){
	 L21PID=11;
	 L22PID=-11;
	 f_lept3_pdgid=11;
	 f_lept4_pdgid=-11;
       ...
       if (safeAccess(RECOELE_CHARGE)[indexleptonfinal[0]] == 1){
	 L11PID=-11;
	 L12PID=+11;
	 f_lept1_pdgid=-11;
	 f_lept2_pdgid=11;
       }
       else if (safeAccess(RECOELE_CHARGE)[indexleptonfinal[0]] == -1){
	 L11PID=11;
	 L12PID=-11;
	 f_lept1_pdgid=11;
	 f_lept2_pdgid=-11;
	 ...
	 float pt_error = 0;
	 vpartP.clear();
	 pt_error = (abs(L11PID) == 13)? pt_error = safeAccess(RECOMU_mubesttrkPTError)[indexleptonfinal[0]] : pt_error = safeAccess(RECOELE_PTError)[indexleptonfinal[0]];
	 ...
	 vpartP.clear();
	 pt_error = (abs(L11PID) == 13)? pt_error = safeAccess(RECOMU_mubesttrkPTError)[indexleptonfinal[1]] : pt_error = safeAccess(RECOELE_PTError)[indexleptonfinal[1]];
	 ...
	 vpartP.clear();
	 pt_error = (abs(L11PID) == 13)? pt_error = safeAccess(RECOMU_mubesttrkPTError)[indexleptonfinal[2]] : pt_error = safeAccess(RECOELE_PTError)[indexleptonfinal[2]];
	 ...
	 vpartP.clear();
	 pt_error = (abs(L11PID) == 13)? pt_error = safeAccess(RECOMU_mubesttrkPTError)[indexleptonfinal[3]] : pt_error = safeAccess(RECOELE_PTError)[indexleptonfinal[3]];
     ...
     if (abs(mass4l-125.)<=10. && N_good==2 && Ne_good==2 && n_bjets<=1 && pass_METFilters) {
       ++N_10;
     ...
     if (abs(mass4l-125.)<=10. && N_good==2 && Ne_good==2 && n_bjets<=1 && RECO_PFMET>=50. && pass_METFilters) {
       ++N_11;
