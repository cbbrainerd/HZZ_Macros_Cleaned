...
int main (int argc, char ** argv){
    if(argc != 5) {
      std::cout << "Usage: ./Analyzer mc_or_data dataset year/config list_of_files" << std::endl;
      exit(2);
    }
    ...
    std::string nome;
    nome+=path;nome+="/analyzer_out_";
    nome+=dataset;
    nome+=".root";
    TFile *outfile=TFile::Open(nome.c_str(),"RECREATE");
    auto analyzer=new Analyzer(chain,outfile,is_mc,configuration);
    analyzer->Loop();
    analyzer->Write();
    outfile->Close();
    //delete tree3;
